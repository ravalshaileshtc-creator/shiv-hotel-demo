...
Tax Rate: ${dbState.settings.taxPercentage}% GST.
...
2. If asked for a recommendation, recommend actual dishes from our menu listed above. Speak highly of our Chef's specials like Paneer Butter Masala or Butter Chicken.
...
5. Do NOT make up menu items. Only recommend items from our actual list.`;
    ...
    if (!apiKey) {
      // Mock Gemini AI response if API key is not present
      ...
      } else if (userMsg.includes('non-veg') || userMsg.includes('chicken') || userMsg.includes('meat')) {
        const nonVegItems = dbState.menu.filter(i => !i.isVeg && i.isAvailable).map(i => `• **${i.name}** (${dbState.settings.currencySymbol}${i.price}) - *${i.description}*`).join('\n');
        reply = `For non-vegetarian mains, our signature dish is the rich and creamy **Butter Chicken** 🍗, or you can try our grilled **Chicken Tikka Angare** as a starter. Here's what we have:\n\n${nonVegItems}\n\nWhat sounds good to you?`;
      } else if (userMsg.includes('recommend') || userMsg.includes('best') || userMsg.includes('special')) {
        reply = `I highly recommend starting with our grilled **Tandoori Paneer Tikka** 🧀 (INR 280), followed by the rich **Butter Chicken** 🍗 (INR 380) with a fluffy hot **Garlic Naan** 🫓 (INR 70). For dessert, the **Sizzling Chocolate Brownie** 🍫 (INR 180) is an absolute must-try!`;
      } else if (userMsg.includes('sweet') || userMsg.includes('dessert') || userMsg.includes('brownie')) {
        reply = `To satisfy your sweet tooth, I highly recommend our **Sizzling Chocolate Brownie** 🍫 (INR 180) served warm with vanilla ice cream, or the classic Indian sweet **Gulab Jamun** 🍯 (INR 110). They are the perfect way to finish your meal!`;
      } else if (userMsg.includes('drink') || userMsg.includes('beverage') || userMsg.includes('lassi')) {
        reply = `We offer refreshing beverages! You must try our creamy **Mango Lassi** 🥭 (INR 120) or our bubbly, custom **Fresh Lime Soda** 🍋 (INR 80) which can be prepared sweet, salted, or mixed!`;
      } else {
        reply = `That sounds delightful! If you're looking for recommendations, our Chef's favorites are the creamy **Paneer Butter Masala** or **Butter Chicken** served with **Garlic Naan**. Let me know if you have any dietary restrictions or need drink pairings! 🍹`;
      }
  ...
  app.listen(PORT, () => {
    console.log(`Shiv Resto server running on http://localhost:${PORT} (${isProduction ? 'Production' : 'Development'})`);
  });
}
